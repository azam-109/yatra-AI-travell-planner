...
export function registerTravelSocket(io) {
  ...
  io.on("connection", (socket) => {
    socket.on("travel:plan", async ({ prompt, chatId, saveTrip = true } = {}) => {
      try {
        if (!prompt) throw new Error("Prompt is required");
        const chat = await getOrCreateChat({ userId: socket.user._id, chatId, prompt });
        await appendMessage({ chatId: chat._id, role: "user", content: prompt });
        socket.emit("travel:started", { chatId: chat._id });
        ...
        const conversationHistory = (chatWithMessages?.messages || [])
          .slice(-10)
          .map((m) => ({
            role: m.role,
            content: m.content,
        ...
        const result = await graph.invoke(
          {
            ...
            prompt,
            conversationHistory,
            userPreferences: socket.user.preferences || {}  // parsed object, not JSON.stringify
            },
        ...
        socket.emit("travel:complete", { chatId: chat._id, trip, saveWarning, result: { ...result, finalResponse } });
      } catch (error) {
        socket.emit("travel:error", { message: error.message || "Planning failed" });
      }
    });
  });
}
